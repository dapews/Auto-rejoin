...
def draw_ui(accounts, phase, detail="", extra=""):
    ...
    def row(a, b, col=RES):
        sys.stdout.write(f"{CYA}│{RES} {trunc(a,C1):<{C1}}{CYA}│{RES} {col}{trunc(b,C2):<{C2}}{RES}{CYA}│{RES}\n")
    ...
    for a in accounts:
        ...
        if any(x in st for x in ['In Game','Online','✅']):
            col = GRE
        elif any(x in st for x in ['Waiting','Starting','Opening','Rejoin','Tunggu']):
            col = YEL
...
def set_low_performance(pkg):
    ...
    xml_content = '''<?xml version='1.0' encoding='utf-8' standalone='yes' ?> <map>     <int name="GraphicsQualityLevel" value="1" />     <int name="SavedQualityLevel" value="1" />     <boolean name="GraphicsAutoQuality" value="false" /> </map>'''
...
def open_ps_link(link, pkg, index=1, total=1):
    ...
    float_flags = f"--windowingMode 5 --bounds {bounds}"
...
def start_rejoin_app():
    ...
    rejoin_timer = cfg.get("rejoin_timer", 3600)
    accounts = [{
        "index":  i + 1,
        ...
        while True:
            for a in accounts:
                ...
                if running:
                    ...
                    if detect_freeze(a["pkg"]):
                        ...
                        elapsed = time.time() - a["last_rejoin"]
                        sisa    = int(rejoin_timer - elapsed)
                        if sisa <= 0:
                            a["status"] = "Rejoin Terjadwal 🔄"
                            log(f"{a['name']}: rejoin terjadwal ({rejoin_timer//60} menit)", "INFO")
                            draw_ui(accounts, "Rejoin Terjadwal", a["name"])
                            click_reconnect(a["pkg"])
                            time.sleep(5)
                            if not is_running(a["pkg"]):
                                clear_cache(a["pkg"])
                                open_ps_link(a["link"], a["pkg"], a["index"], tot)
                                protect_app(a["pkg"])
                                set_low_performance(a["pkg"])
                                for t in range(load_wait, 0, -1):
                                    a["status"] = f"Load Ulang ({t}s)"
                                    draw_ui(accounts, "Rejoin", a["name"])
                                    time.sleep(1)
                            a["last_rejoin"] = time.time()
                            a["status"] = "In Game ✅" if is_running(a["pkg"]) else "Gagal ❌"
                            a["fail"] = 0
                        else:
                            menit = sisa // 60
                            detik = sisa % 60
                            a["status"] = f"In Game ✅ | Rejoin: {menit}m{detik}s"
                            a["fail"] = 0
                else:
                    a["fail"] += 1
                    if a["fail"] >= 2:
                        a["status"] = "Force Close! Tunggu 60s... 🔄"
            ...
            try:
                ...
                with open("status.json", "w") as f:
                    _json.dump([{"name": x["name"], "status": x["status"]} for x in accounts], f)
            except:
                pass
...
def create_config():
    ...
    rt = clean_input("  Rejoin terjadwal tiap berapa menit? (default 60): ").strip()
    config = {
        ...
        "rejoin_timer":   (int(rt) * 60) if rt.isdigit() else 3600,
        "accounts":       accounts,
    }
